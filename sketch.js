...
let pg;
let colorfondo;
let textomuelle = 'El muelle es el principal acceso a la isla, en su momento era el punto de descarga de materiales de contruccion. Hoy en dia es utilizado por veleros y otras embarcaciones para llegar a la isla.';
let textoreactor = 'El reactor es la estructura mas ambiociosa de toda la isla. En su interior se desarrollaria el principal experimento: la fusion nuclear.'
let current;
let opacidad = 255;
let topo = 0;
...
function setup() {
  ...
  easycam = new Dw.EasyCam(this._renderer, {distance:1000, center:[0,0,0]});
...
function draw() {
  ...
  image(grillamapa, 0, 0,1600, 800);  // fondo grilla
    ...
    if (topo == 0) {booleantopo = false}
    else if (topo == 1) {booleantopo = true}
...
text("El muelle",380, 494);
 ...
 push ();
 tint (255,fade),
  image(reactorcenital, 640, 310, 70,70);
  pop ();
...
if (easycam.state.distance <= 250 && easycam.state.center[0] <= -420 && easycam.state.center[0] >= -520 && easycam.state.center[1] >= 40 && easycam.state.center[1] <= 120  ) {
...
// USINA HUD //
if (easycam.state.distance <= 250 && easycam.state.center[0] <= -80 && easycam.state.center[0] >= -200 && easycam.state.center[1] >= -300 && easycam.state.center[1] <= -250  ) {
...
if (easycam.state.distance <= 250 && easycam.state.center[0] <= -60 && easycam.state.center[0] >= -160 && easycam.state.center[1] >= -130 && easycam.state.center[1] <= -80) {
