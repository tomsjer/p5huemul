...
export default class TableroVista {
  constructor(config) {
    ...
    this.aspectRatio =
      this.modelo.estadoGrilla[0].length / this.modelo.estadoGrilla.length;
    ...
    this.comida.rect(
      this.anchoCelda / 2 - this.radioComida / 2,
      this.altoCelda / 2 - this.radioComida / 2,
      this.radioComida,
      this.radioComida
    );
    ...
    this.bola = new Path2D(`M${this.anchoCelda / 2 - u * 3} ${
      this.altoCelda / 2 - u * 3
    }
    h ${u} v -${u} h ${u} v -${u} h${u * 2} v ${u} h ${u} v${u} h ${u} v ${
      u * 2
    } h -${u} v ${u} h -${u} v ${u} h -${
      u * 2
    } v -${u} h -${u} v -${u} h -${u} v -${u * 2}`);
    ...
    this.grillaFondo = this.setGrillaFondo.bind(this)(
      this.modelo.filas,
      this.modelo.columnas
    );
    this.grillaJuego = this.setGrillaJuego.bind(this)(
      this.modelo.filas,
      this.modelo.columnas
    );
  ...
  setGrillaFondo(_f, _c) {
    return this.setGrilla(
      _f,
      _c,
      (estado) => this.estadosJuego.indexOf(estado) === -1
    );
  }
